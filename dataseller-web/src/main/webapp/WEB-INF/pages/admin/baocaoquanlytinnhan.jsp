...
<c:url var="formUrl" value="${prefix}/bcquanlytinnhan.html"/>
<body>
                ...
                <form:form commandName="item" id="reportForm" action="${formUrl}" method="post" autocomplete="off">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-8">
                                        <form:select id="loaiDoiTuongMenu" path="loaiTinNhanTuKHCN" cssStyle="width: 200px;">
                                            ...
                                            <option <c:if test="${item.loaiTinNhanTuKHCN eq Constants.MOBIFONE_DK}">selected="true"</c:if> value="${Constants.MOBIFONE_DK}">${Constants.MOBIFONE_DK}</option>
                                            <option <c:if test="${item.loaiTinNhanTuKHCN eq Constants.MOBIFONE_HUY}">selected="true"</c:if> value="${Constants.MOBIFONE_HUY}">${Constants.MOBIFONE_HUY}</option>
                                            <option <c:if test="${item.loaiTinNhanTuKHCN eq Constants.MOBIFONE_KT}">selected="true"</c:if> value="${Constants.MOBIFONE_KT}">${Constants.MOBIFONE_KT}</option>
                                            <option <c:if test="${item.loaiTinNhanTuKHCN eq Constants.MOBIFONE_DP}">selected="true"</c:if> value="${Constants.MOBIFONE_DP}">${Constants.MOBIFONE_DP}</option>
                                        </form:select>
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-8">
                                        <form:select id="loaiDoiTuongMenu" path="loaiTinNhanTuVMS" cssStyle="width: 200px;">
                                            ...
                                            <option <c:if test="${item.loaiTinNhanTuVMS eq Constants.LOAITINNHAN_4_1}">selected="true"</c:if> value="${Constants.LOAITINNHAN_4_1}">${Constants.LOAITINNHAN_4_1}</option>
                                            <option <c:if test="${item.loaiTinNhanTuVMS eq Constants.LOAITINNHAN_4_2}">selected="true"</c:if> value="${Constants.LOAITINNHAN_4_2}">${Constants.LOAITINNHAN_4_2}</option>
                    ...
                    <table id="tableNote">
                        <tr>
                            ...
                            <td>MobiFone thuc hien CT "Tich diem cuoc goi, nhan ngay voucher" den 20/12/2014 : Tang 1 phieu mua hang 50.000d Gift Pass hoặc phieu Gift Network khi tich luy du 100 diem (1 diem = 1.000d tru trong Tai khoan chinh).CT chi ap dung tai Tp.Ho Chi Minh.</td>
                        </tr>
                        <tr>
                            <td><span class="text-note">${Constants.LOAITINNHAN_2_1}</span></td>
                            <td>Quy khach da dong y tham gia CT "Tich diem cuoc goi, nhan ngay voucher": MobiFone tang 1 phieu mua hang 50.000d Gift Pass hoặc phieu Gift Network khi Quy khach tich luy du 100 diem (1 diem = 1.000d tru trong Tai khoan chinh). CT den 20/12/2014, ap dung tai Tp. Ho Chi Minh.</td>
                        </tr>
                        <tr>
                            <td><span class="text-note">${Constants.LOAITINNHAN_2_2}</span></td>
                            <td>Cu phap dang ky cua Quy khach khong dung. Soan tin nhan gui 9234 de tham gia CT "Tich diem cuoc goi, nhan ngay voucher" theo cu phap: MOBIFONE DK</td>
                        </tr>
                        <tr>
                            <td><span class="text-note">${Constants.LOAITINNHAN_3}</span></td>
                            <td>Quy khach khong thuoc doi tuong tham gia CT "Tich diem cuoc goi, nhan ngay voucher". Lien he 9090 de biet them chi tiet.</td>
                        </tr>
                        <tr>
                            <td><span class="text-note">${Constants.LOAITINNHAN_4_1}</span></td>
                            <td>Quy khach da tich luy duoc X diem trong Chuong trinh "Tich diem cuoc goi, nhan ngay voucher" tinh den het ngay N (Sau khi doi Ma). Tiep tuc tich luy diem de doi phieu mua hang tri gia 50.000d. Chi tiet L/H 9090.</td>
                        </tr>
                        <tr>
                            <td><span class="text-note">${Constants.LOAITINNHAN_4_2}</span></td>
                            <td>Quy khach da tich luy duoc X diem trong Chuong trinh "Tich diem cuoc goi, nhan ngay voucher" tinh den het ngay N (Sau khi doi Ma). Tiep tuc tich luy them Y diem (su dung Y ngan dong TKC) de doi phieu mua hang tri gia 50.000d. Chi tiet L/H 9090</td>
                        </tr>
                        <tr>
                            <td><span class="text-note">${Constants.LOAITINNHAN_5}</span></td>
                            <td>Hay goi va nhan tin de tich luy diem va doi phieu mua hang 50.000d. Den het ngay N, Quy khach da tich luy duoc X diem trong CT "Tich diem cuoc goi, nhan ngay voucher". Chi tiet lien he 9090.</td>
                        </tr>
                        <tr>
                            <td><span class="text-note">${Constants.LOAITINNHAN_6}</span></td>
                            <td>Quy khach da tich luy du diem de nhan qua tang mien phi tu MobiFone. Soan: MOBIFONE DP gui 9234 hoac Reply (Tra loi): MOBIFONE DP de nhan ma so doi qua tang.</td>
                        </tr>
                        <tr>
                            <td><span class="text-note">${Constants.LOAITINNHAN_7_1}</span></td>
                            <td>Ma doi phieu mua hang cua Chuong trinh "Tich diem cuoc goi, nhan ngay voucher" la x1x2x3x4x5x6x7x8x9x10x11x12. Vui long den cac cua hang MobiFone tai Tp. Ho Chi Minh de doi phieu mua hang tri gia 50.000d truoc 31/12/2014. Luu y: CT se ket thuc som neu so luong phieu mua hang duoc doi het. Chi tiet L/H 9090.</td>
                        </tr>
                        <tr>
                            <td><span class="text-note">${Constants.LOAITINNHAN_7_2}</span></td>
                            <td>Ma doi thuong cua Chuong trinh "Tich diem cuoc goi, nhan ngay voucher" tu : x1x2x3x4x5x6x7x8x9x10x11x12 den ,y1y2y3y4y5y6y7y8y9y10y11y12. Vui long den cac cua hang MobiFone tai Tp. Ho Chi Minh de doi phieu mua hang truoc 31/12/2014. Luu y: CT se ket thuc som neu so luong phieu mua hang duoc doi het. Chi tiet L/H 9090.</td>
                        </tr>
                        <tr>
...
<script type="text/javascript">
    $(document).ready(function(){
        setActiveMenu4Admin('#tdcg2014ReportQuanLyTinNhanTab');
