 ...
 */
@Controller
public class BaoCaoTongHopKetQuaChuongTrinh_TheoChiNhanhController extends ApplicationObjectSupport {
    private Log log = LogFactory.getLog(BaoCaoTongHopKetQuaChuongTrinh_TheoChiNhanhController.class);
    ...
    @RequestMapping(value = {"/admin/thuebaophattrienmoi/baocaotonghop_ketquachuongtrinh_theochinhanh.html",
            "/chinhanh/thuebaophattrienmoi/baocaotonghop_ketquachuongtrinh_theochinhanh.html",
            "/baocao/thuebaophattrienmoi/baocaotonghop_ketquachuongtrinh_theochinhanh.html"})
    public ModelAndView report(@ModelAttribute(value = Constants.FORM_MODEL_KEY)BaoCaoTongHopKetQuaChuongTrinhCommand command,
        ...
        ModelAndView mav = new ModelAndView("2015/thuebaophattrienmoi/admin/baocaotonghopketquachuongtrinh/theochinhanh");
        ...
        if (StringUtils.isNotBlank(crudaction)){
            if (crudaction.equals("search")){
                ...
                Object[] results = this.baoCaoTongHopKetQuaChuongTrinhService.baoCaoTongHopKetQuaChuongTrinh_TheoChiNhanh(properties, command.getSortExpression(), command.getSortDirection(), command.getFirstItem(), command.getReportMaxPageItems());
    ...
    private void export2Excel(BaoCaoTongHopKetQuaChuongTrinhCommand command, HttpServletRequest request, HttpServletResponse response) throws Exception {
        try{
            ...
            Map<String, Object> properties = buildProperties(command);
            Object[] resultObject = this.baoCaoTongHopKetQuaChuongTrinhService.baoCaoTongHopKetQuaChuongTrinh_TheoChiNhanh(properties, command.getSortExpression(), command.getSortDirection(), 0, Integer.MAX_VALUE);
            List<BaoCaoTongHopKetQuaChuongTrinhDTO> dtoList = (List<BaoCaoTongHopKetQuaChuongTrinhDTO>)resultObject[1];

            String outputFileName = "/files/temp/baocaotonghopketquachuongtrinh_theochinhanh" + ngayXuatBaoCao + ".xls";
            String reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/export/thuebaoptm_2015/baocaotonghopketquachuongtrinh_theochinhanh.xls");
    ...
    private CellValue[] addCellValues(BaoCaoTongHopKetQuaChuongTrinhDTO dto, int indexRow) {
        ...
        if(dto.getSLDBHThamGia() != null){
            resValue[columnIndex++] = new CellValue(CellDataType.STRING, dto.getSLDBHThamGia());
        }else{
            resValue[columnIndex++] = new CellValue();
        }
