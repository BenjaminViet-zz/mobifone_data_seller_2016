...
import com.benluck.vms.mobifonedataseller.webapp.command.command2014.BaoCaoTinNhan_kpp_theoChiNhanhCommand;
 ...
 */
@Controller
public class BaoCaoTinNhan_kpp_theoChiNhanhController extends ApplicationObjectSupport {
    private Log log = LogFactory.getLog(BaoCaoTinNhan_kpp_theoChiNhanhController.class);
    private static final Integer TOTAL_COLUMN_EXPORT = 9;

    @Autowired
    private BaoCaoTinNhan_kpp_theoChiNhanhManagementLocalBean reportManagementLocalBean;
    @Autowired
    ...
    @RequestMapping(value = {"/admin/baocaotinnhan_kpp_theochinhanh.html","/chinhanh/baocaotinnhan_kpp_theochinhanh.html"})
    public ModelAndView report(@ModelAttribute(value = Constants.FORM_MODEL_KEY)BaoCaoTinNhan_kpp_theoChiNhanhCommand command,
                               HttpServletRequest request,
                               HttpServletResponse response){
        ModelAndView mav = new ModelAndView("/admin/baocaotinnhan_kpp/theochinhanh");
        ...
        if(StringUtils.isNotBlank(crudaction)){
            if(crudaction.equals("search")){
                ...
                command.setListResult((List<BaoCaoTinNhan_KPP_TheoChiNhanhDTO>)resultObject[1]);
    ...
    private void referenceData(BaoCaoTinNhan_kpp_theoChiNhanhCommand command, ModelAndView mav){
    ...
    private Map<String, Object> buildProperties(BaoCaoTinNhan_kpp_theoChiNhanhCommand command){
    ...
    private void convertDate2Timestamp(BaoCaoTinNhan_kpp_theoChiNhanhCommand command){
    ...
    private void export2Excel(BaoCaoTinNhan_kpp_theoChiNhanhCommand command, HttpServletRequest request,HttpServletResponse response)throws Exception{
        try{
            ...
            List<BaoCaoTinNhan_KPP_TheoChiNhanhDTO> dtoList = (List<BaoCaoTinNhan_KPP_TheoChiNhanhDTO>)resultObject[1];
            String reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/export/baocaotinnhan_kpp_theochinhanh.xls");
            String outputFileName = "/files/temp/baocaotinnhan_kpp_theochinhanh_" + ngayXuatBaoCao + ".xls";
            ...
            if(dtoList.size() > 0){
                int indexRow = 1;
                for(BaoCaoTinNhan_KPP_TheoChiNhanhDTO dto : dtoList){
            ...
            throw new Exception("Failed to export to excel BaoCaoTinNhanKPPTheoChiNhanh. \n" + e.getMessage());
        }
    }

    private CellValue[] addCellValues(BaoCaoTinNhan_KPP_TheoChiNhanhDTO dto, int indexRow){
        SimpleDateFormat df = new SimpleDateFormat("dd-M-yyyy");
