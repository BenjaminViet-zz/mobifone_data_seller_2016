 ...
 */
@Controller
public class BaoCaoTinNhanTheoDailyController extends ApplicationObjectSupport {
    private Log log = LogFactory.getLog(BaoCaoTinNhanTheoDailyController.class);
    ...
    @RequestMapping(value = {"/admin/thuebaophattrienmoi/baocaotinnhan_theodaily.html",
        "/chinhanh/thuebaophattrienmoi/baocaotinnhan_theodaily.html",
            "/baocao/thuebaophattrienmoi/baocaotinnhan_theodaily.html"})
    public ModelAndView report(@ModelAttribute(value = Constants.FORM_MODEL_KEY)BaoCaoTinNhanCommand command,
        ...
        ModelAndView mav = new ModelAndView("2015/thuebaophattrienmoi/admin/baocaotinnhan/theodaily");
        ...
        if (StringUtils.isNotBlank(crudaction)){
            if (crudaction.equals("search")){
                ...
                Object[] results = this.baoCaoTinNhanService.baoCaoTinNhan_TheoDiemBanHang(properties, command.getSortExpression(), command.getSortDirection(), command.getFirstItem(), command.getReportMaxPageItems());
    ...
    private void export2Excel(BaoCaoTinNhanCommand command, HttpServletRequest request, HttpServletResponse response) throws Exception {
        try{
            ...
            Map<String, Object> properties = buildProperties(command);
            Object[] resultObject = this.baoCaoTinNhanService.baoCaoTinNhan_TheoDiemBanHang(properties, command.getSortExpression(), command.getSortDirection(), 0, Integer.MAX_VALUE);
            List<BaoCaoTinNhanDTO> dtoList = (List<BaoCaoTinNhanDTO>)resultObject[1];

            String outputFileName = "/files/temp/baocaotinnhan_theodaily" + ngayXuatBaoCao + ".xls";
            String reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/export/thuebaoptm_2015/baocaotinnhan_theodaily.xls");
            ...
            if(command.getFromDate() != null){
                ...
                resValueNgayXuatBaoCaoTu[7] = new CellValue(CellDataType.STRING, "Ngày xuất báo cáo từ: " + df.format(command.getFromDate()));
                ExcelUtil.addRow(sheet, 4, resValueNgayXuatBaoCaoTu, stringNgayBaoCaoCellFormat, null, null, null);
            }
            if(command.getToDate() != null){
                CellValue[] resValueNgayXuatBaoCaoTu = WebCommonUtil.addNgayXuatBaoCao(TOTAL_COLUMN_EXPORT);
                resValueNgayXuatBaoCaoTu[7] = new CellValue(CellDataType.STRING, "Ngày xuất báo cáo đến: " + df.format(command.getToDate()));
