...
import com.benluck.vms.mobifonedataseller.webapp.command.command2014.BaoCaoTinNhan_kpp_theoQuanHuyenCommand;
 ...
 */
@Controller
public class BaoCaoTinNhan_kpp_theoQuanHuyenController extends ApplicationObjectSupport {
    private Log log = LogFactory.getLog(BaoCaoTinNhan_kpp_theoQuanHuyenController.class);
    private static final Integer TOTAL_COLUMN_EXPORT = 10;

    @Autowired
    private BaoCaoTinNhan_kpp_theoQuanHuyenManagementLocalBean reportManagementLocalBean;
    @Autowired
    ...
    @RequestMapping(value = {"/admin/baocaotinnhan_kpp_theoquanhuyen.html","/chinhanh/baocaotinnhan_kpp_theoquanhuyen.html"})
    public ModelAndView report(@ModelAttribute(value = Constants.FORM_MODEL_KEY)BaoCaoTinNhan_kpp_theoQuanHuyenCommand command,
                               HttpServletRequest request,
                               HttpServletResponse response){
        ModelAndView mav = new ModelAndView("/admin/baocaotinnhan_kpp/theoquanhuyen");
        ...
        if(StringUtils.isNotBlank(crudaction)){
            if(crudaction.equals("search")){
                ...
                command.setListResult((List<BaoCaoTinNhan_KPP_TheoQuanHuyenDTO>)resultObject[1]);
     ...
     * @param mav
     */
    private void referenceData(BaoCaoTinNhan_kpp_theoQuanHuyenCommand command, ModelAndView mav){
     ...
     * @return
     */
    private Map<String, Object> buildProperties(BaoCaoTinNhan_kpp_theoQuanHuyenCommand command){
     ...
     * @param command
     */
    private void convertDate2Timestamp(BaoCaoTinNhan_kpp_theoQuanHuyenCommand command){
     ...
     * @throws Exception
     */
    private void export2Excel(BaoCaoTinNhan_kpp_theoQuanHuyenCommand command, HttpServletRequest request,HttpServletResponse response)throws Exception{
        try{
            ...
            List<BaoCaoTinNhan_KPP_TheoQuanHuyenDTO> dtoList = (List<BaoCaoTinNhan_KPP_TheoQuanHuyenDTO>)resultObject[1];
            String reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/export/baocaotinnhan_kpp_theoquanhuyen.xls");
            String outputFileName = "/files/temp/baocaotinnhan_kpp_theoquanhuyen_" + ngayXuatBaoCao + ".xls";
            ...
            if(dtoList.size() > 0){
                int indexRow = 1;
                for(BaoCaoTinNhan_KPP_TheoQuanHuyenDTO dto : dtoList){
            ...
            throw new Exception("Failed to export to excel BaoCaoTinNhanKPPTheoQuanHuyen. \n" + e.getMessage());
        }
    }
     ...
     * @return
     */
    private CellValue[] addCellValues(BaoCaoTinNhan_KPP_TheoQuanHuyenDTO dto, int indexRow){
        SimpleDateFormat df = new SimpleDateFormat("dd-M-yyyy");
