...
import com.benluck.vms.mobifonedataseller.webapp.command.command2014.BaoCaoChiTietHangMucPSC_kpp_muaBTGTuMobiFone_theoChiNhanhCommand;
 ...
 */
@Controller
public class BaoCaoChiTietHangMucPSC_kpp_muaBTGTuMobiFone_theoChiNhanhController extends ApplicationObjectSupport {
    private Log log = LogFactory.getLog(BaoCaoChiTietHangMucPSC_kpp_muaBTGTuMobiFone_theoChiNhanhController.class);
    private static final Integer TOTAL_COLUMN_EXPORT = 7;

    @Autowired
    private BaoCaoChiTietHangMucPSC_kpp_mua_btg_tu_mobifone_theoChiNhanh_managementLocalBean reportManagementLocalBean;
    @Autowired
    ...
    @RequestMapping(value = {"/admin/baocaochitiethangmucpsc_muaBTGTuMobifone_theochinhanh.html","/chinhanh/baocaochitiethangmucpsc_muaBTGTuMobifone_theochinhanh.html"})
    public ModelAndView report(@ModelAttribute(value = Constants.FORM_MODEL_KEY)BaoCaoChiTietHangMucPSC_kpp_muaBTGTuMobiFone_theoChiNhanhCommand command,
                               HttpServletRequest request,
                               HttpServletResponse response){
        ModelAndView mav = new ModelAndView("/admin/baocaochitiethangmucpsc/muabtgtumobifone/theochinhanh");
        ...
        if(StringUtils.isNotBlank(crudaction)){
            if(crudaction.equals("search")){
                ...
                command.setListResult((List<BaoCaoChiTietHangMucPSCKPP_MuaBTGTuMobiFone_TheoChiNhanhDTO>)resultObject[1]);
    ...
    private void referenceData(BaoCaoChiTietHangMucPSC_kpp_muaBTGTuMobiFone_theoChiNhanhCommand command, ModelAndView mav){
    ...
    private Map<String, Object> buildProperties(BaoCaoChiTietHangMucPSC_kpp_muaBTGTuMobiFone_theoChiNhanhCommand command){
    ...
    private void convertDate2Timestamp(BaoCaoChiTietHangMucPSC_kpp_muaBTGTuMobiFone_theoChiNhanhCommand command){
    ...
    private void export2Excel(BaoCaoChiTietHangMucPSC_kpp_muaBTGTuMobiFone_theoChiNhanhCommand command, HttpServletRequest request,HttpServletResponse response)throws Exception{
        try{
            ...
            List<BaoCaoChiTietHangMucPSCKPP_MuaBTGTuMobiFone_TheoChiNhanhDTO> dtoList = (List<BaoCaoChiTietHangMucPSCKPP_MuaBTGTuMobiFone_TheoChiNhanhDTO>)resultObject[1];
            String reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/export/baocaochitiethangmuc_psc_kpp_mua_btg_tu_mobifone_theochinhanh.xls");
            String outputFileName = "/files/temp/baocaochitiethangmuc_psc_kpp_mua_btg_tu_mobifone_theochinhanh_" + ngayXuatBaoCao + ".xls";
            ...
            if(dtoList.size() > 0){
                int indexRow = 1;
                for(BaoCaoChiTietHangMucPSCKPP_MuaBTGTuMobiFone_TheoChiNhanhDTO dto : dtoList){
    ...
    private CellValue[] addCellValues(BaoCaoChiTietHangMucPSCKPP_MuaBTGTuMobiFone_TheoChiNhanhDTO dto, int indexRow){
        SimpleDateFormat df = new SimpleDateFormat("dd-M-yyyy");
