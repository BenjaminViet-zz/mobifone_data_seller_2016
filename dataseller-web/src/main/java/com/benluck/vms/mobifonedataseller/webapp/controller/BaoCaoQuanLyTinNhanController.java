 ...
 */
@Controller
public class BaoCaoQuanLyTinNhanController extends ApplicationObjectSupport{
    ...
    @RequestMapping(value = {"/admin/bcquanlytinnhan.html","/chinhanh/bcquanlytinnhan.html"})
    public ModelAndView bcQuanLyTinNhan(@ModelAttribute(value = Constants.FORM_MODEL_KEY)BaoCaoQuanLyTinNhanCommand command,
        ...
        ModelAndView mav = new ModelAndView("/admin/baocaoquanlytinnhan");
        ...
        List<BaoCaoQuanLyTinNhanDTO> dtoList = this.reportCtTichDiemManagementLocalBean.baoCaoQuanLyTinNhan_tdcg(command.getLoaiTinNhanTuKHCN(), command.getLoaiTinNhanTuVMS(), command.getFromDateTime(), command.getToDateTime());
    ...
    private void export2Excel(List<BaoCaoQuanLyTinNhanDTO> dtoList, Timestamp fromDate, Timestamp toDate, HttpServletRequest request, HttpServletResponse response)throws Exception{
        try{
            ...
            String reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/export/baocaoquanlytinnhan.xls");
