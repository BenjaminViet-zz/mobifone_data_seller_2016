...
import com.benluck.vms.mobifonedataseller.core.business.BaoCaoChiTietHangMucPSC_kpp_mua_the_tu_mobifone_theoDBH_managementLocalBean;
...
import com.benluck.vms.mobifonedataseller.webapp.command.command2014.BaoCaoChiTietHangMucPSC_kpp_muaTheTuMobiFone_theoDBHCommand;
 ...
 */
@Controller
public class BaoCaoChiTietHangMucPSC_kpp_muaTheTuMobiFone_theoDBHController extends ApplicationObjectSupport {
    private Log log = LogFactory.getLog(BaoCaoChiTietHangMucPSC_kpp_muaTheTuMobiFone_theoDBHController.class);
    private static final Integer TOTAL_COLUMN_EXPORT = 10;

    @Autowired
    private BaoCaoChiTietHangMucPSC_kpp_mua_the_tu_mobifone_theoDBH_managementLocalBean reportManagementLocalBean;
    @Autowired
    ...
    @RequestMapping(value = {"/admin/baocaochitiethangmucpsc_mua_the_tu_Mobifone_theodaily.html","/chinhanh/baocaochitiethangmucpsc_mua_the_tu_Mobifone_theodaily.html"})
    public ModelAndView report(@ModelAttribute(value = Constants.FORM_MODEL_KEY)BaoCaoChiTietHangMucPSC_kpp_muaTheTuMobiFone_theoDBHCommand command,
                               HttpServletRequest request,
                               HttpServletResponse response){
        ModelAndView mav = new ModelAndView("/admin/baocaochitiethangmucpsc/muathetumobifone/theodaily");
        ...
        if(StringUtils.isNotBlank(crudaction)){
            if(crudaction.equals("search")){
                ...
                command.setListResult((List<BaoCaoChiTietHangMucPSCKPP_MuaTheTuMobiFone_TheoDBHDTO>)resultObject[1]);
    ...
    private void referenceData(BaoCaoChiTietHangMucPSC_kpp_muaTheTuMobiFone_theoDBHCommand command, ModelAndView mav){
    ...
    private Map<String, Object> buildProperties(BaoCaoChiTietHangMucPSC_kpp_muaTheTuMobiFone_theoDBHCommand command){
    ...
    private void convertDate2Timestamp(BaoCaoChiTietHangMucPSC_kpp_muaTheTuMobiFone_theoDBHCommand command){
    ...
    private void export2Excel(BaoCaoChiTietHangMucPSC_kpp_muaTheTuMobiFone_theoDBHCommand command, HttpServletRequest request,HttpServletResponse response)throws Exception{
        try{
            ...
            List<BaoCaoChiTietHangMucPSCKPP_MuaTheTuMobiFone_TheoDBHDTO> dtoList = (List<BaoCaoChiTietHangMucPSCKPP_MuaTheTuMobiFone_TheoDBHDTO>)resultObject[1];
            String reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/export/baocaochitiethangmuc_psc_kpp_mua_the_tu_mobifone_theodaily.xls");
            String outputFileName = "/files/temp/baocaochitiethangmuc_psc_kpp_mua_the_tu_mobifone_theodaily_" + ngayXuatBaoCao + ".xls";
            ...
            if(dtoList.size() > 0){
                int indexRow = 1;
                for(BaoCaoChiTietHangMucPSCKPP_MuaTheTuMobiFone_TheoDBHDTO dto : dtoList){
    ...
    private CellValue[] addCellValues(BaoCaoChiTietHangMucPSCKPP_MuaTheTuMobiFone_TheoDBHDTO dto, int indexRow){
        SimpleDateFormat df = new SimpleDateFormat("dd-M-yyyy");
