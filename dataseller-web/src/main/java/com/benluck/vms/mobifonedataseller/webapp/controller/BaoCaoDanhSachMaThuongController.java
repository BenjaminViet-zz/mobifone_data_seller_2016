...
import com.benluck.vms.mobifonedataseller.webapp.command.command2014.BaoCaoDanhSachMaThuongCommand;
 ...
 */
@Controller
public class BaoCaoDanhSachMaThuongController extends ApplicationObjectSupport {
    private Log log = LogFactory.getLog(BaoCaoDanhSachMaThuongController.class);
    ...
    @Autowired
    private BaoCaoDanhSachMaThuongManagementLocalBean baoCaoDanhSachMaThuongManagementLocalBean;
    @Autowired
    ...
    @RequestMapping(value = {"/admin/baocaodanhsachmathuong.html","/chinhanh/baocaodanhsachmathuong.html"})
    public ModelAndView baoCaoDanhSachMaThuong(@ModelAttribute(value = Constants.FORM_MODEL_KEY)BaoCaoDanhSachMaThuongCommand command,
                                               HttpServletRequest request,
                                               HttpServletResponse response){
        ModelAndView mav = new ModelAndView("/admin/baocaodanhsachmathuong");
        ...
        if(StringUtils.isNotBlank(crudaction)){
            if(crudaction.equals("search")){
                ...
                Object[] resultObject = this.baoCaoDanhSachMaThuongManagementLocalBean.getWinningCodeList(properties, command.getFirstItem(), command.getReportMaxPageItems(), command.getSortExpression(), command.getSortDirection());
                command.setTotalItems(Integer.valueOf(resultObject[0].toString()));
                command.setListResult((List<BaoCaoDanhSachMaThuongDTO>)resultObject[1]);
        ...
        if(SecurityUtils.userHasAuthority(Constants.ADMIN_ROLE) || SecurityUtils.userHasAuthority(Constants.BAOCAO_ROLE)){
            List<BranchDTO> branchList = this.baoCaoDanhSachMaThuongManagementLocalBean.findAllBranches();
            ...
            try{
                BranchDTO branchDTO = this.baoCaoDanhSachMaThuongManagementLocalBean.findBranchById(SecurityUtils.getPrincipal().getBranchId());
                List<BranchDTO> branchList = new ArrayList<BranchDTO>();
                branchList.add(branchDTO);
                mav.addObject("branchList", branchList);
            }catch (Exception e){}

            List<DistrictDTO> districtList = this.baoCaoDanhSachMaThuongManagementLocalBean.findByBranchId(SecurityUtils.getPrincipal().getBranchId());
            mav.addObject("districtList", districtList);
        ...
        List<PromItemsDTO> itemList = this.baoCaoDanhSachMaThuongManagementLocalBean.findAllItems();
        mav.addObject("itemList", itemList);
        return  mav;
    }

    private Map<String, Object> buildProperties(BaoCaoDanhSachMaThuongCommand command){
    ...
    private void convertDate2Timestamp(BaoCaoDanhSachMaThuongCommand command){
    ...
    private void export2Excel(BaoCaoDanhSachMaThuongCommand command, HttpServletRequest request,HttpServletResponse response)throws Exception{
        try{
            ...
            convertDate2Timestamp(command);
            Map<String, Object> properties = buildProperties(command);
            Object[] resultObject = this.baoCaoDanhSachMaThuongManagementLocalBean.getWinningCodeList(properties, 0, Integer.MAX_VALUE, command.getSortExpression(), command.getSortDirection());
            List<BaoCaoDanhSachMaThuongDTO> dtoList = (List<BaoCaoDanhSachMaThuongDTO>)resultObject[1];
            String outputFileName = "/files/temp/baocaodanhsachmathuong_" + ngayXuatBaoCao + ".xls";
            String reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/export/baocaodanhsachmathuong.xls");
            ...
            if(dtoList.size() > 0){
                int indexRow = 1;
                for(BaoCaoDanhSachMaThuongDTO dto : dtoList){
    ...
    private CellValue[] addCellValues(BaoCaoDanhSachMaThuongDTO dto, int indexRow){
        SimpleDateFormat df = new SimpleDateFormat("dd/M/yyyy");
        ...
        if(StringUtils.isNotBlank(dto.getSo_EZ())){
            resValue[columnIndex++] = new CellValue(CellDataType.STRING, dto.getSo_EZ());
        }else{
            resValue[columnIndex++] = new CellValue();
        }
        if(StringUtils.isNotBlank(dto.getMa_So())){
            resValue[columnIndex++] = new CellValue(CellDataType.STRING, dto.getMa_So());
        }else{
            resValue[columnIndex++] = new CellValue();
        }
        StringBuilder trang_thai_trung_thuong = null;
        if(dto.getTrang_thai_trung_thuong() != null){
            if(dto.getTrang_thai_trung_thuong().equals(Constants.KPP_MA_THUONG_TRUNG_THUONG)){
                trang_thai_trung_thuong = new StringBuilder(this.getMessageSourceAccessor().getMessage("admin.baocaodanhsachmathuong.label.ma_thuong_trung_thuong"));
            }else if(dto.getTrang_thai_trung_thuong().equals(Constants.KPP_MA_THUONG_KHONG_TRUNG_THUONG)){
                trang_thai_trung_thuong = new StringBuilder(this.getMessageSourceAccessor().getMessage("admin.baocaodanhsachmathuong.label.ma_thuong_khong_trung_thuong"));
            }else if(dto.getTrang_thai_trung_thuong().equals(Constants.KPP_MA_THUONG_CHUA_QUAY_SO)){
                trang_thai_trung_thuong = new StringBuilder(this.getMessageSourceAccessor().getMessage("admin.baocaodanhsachmathuong.label.ma_thuong_chua_quay_so"));
            }
        }else{
            trang_thai_trung_thuong = new StringBuilder(this.getMessageSourceAccessor().getMessage("admin.baocaodanhsachmathuong.label.ma_thuong_chua_quay_so"));
        }
        resValue[columnIndex++] = new CellValue(CellDataType.STRING, trang_thai_trung_thuong.toString());

        if(StringUtils.isNotBlank(dto.getDat_giai())){
            resValue[columnIndex++] = new CellValue(CellDataType.STRING, dto.getDat_giai());
        }else{
            resValue[columnIndex++] = new CellValue();
        }
        return resValue;
