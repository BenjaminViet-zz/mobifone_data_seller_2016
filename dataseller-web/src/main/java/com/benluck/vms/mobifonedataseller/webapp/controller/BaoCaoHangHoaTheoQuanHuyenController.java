 ...
 */
@Controller
public class BaoCaoHangHoaTheoQuanHuyenController extends ApplicationObjectSupport{
    private Log log = LogFactory.getLog(BaoCaoHangHoaTheoQuanHuyenController.class);
    ...
    @RequestMapping(value = {"/admin/baocaohanghoatheoquanhuyen.html","/chinhanh/baocaohanghoatheoquanhuyen.html"})
    public ModelAndView baocaohanghoatheodaily(@ModelAttribute(value = Constants.FORM_MODEL_KEY)BaoCaoHangHoaCommand command,
        ...
        ModelAndView mav = new ModelAndView("/admin/baocaohanghoatheoquanhuyen");
        ...
        if(StringUtils.isNotBlank(crudaction)){
            if(crudaction.equals("search")){
                ...
                Object[] resultobject = this.baoCaoHangHoaManagementLocalBean.baoCaoHangHoaTheoQuanHuyen(properties, command.getFirstItem(), command.getReportMaxPageItems(), command.getSortExpression(), command.getSortDirection());
    ...
    private void export2Excel(BaoCaoHangHoaCommand command, HttpServletRequest request,HttpServletResponse response)throws Exception{
        try{
            ...
            convertDate2Timestamp(command);
            Map<String, Object> properties = buildProperties(command);
            Object[] resultObject = this.baoCaoHangHoaManagementLocalBean.baoCaoHangHoaTheoQuanHuyen(properties, 0, Integer.MAX_VALUE, command.getSortExpression(), command.getSortDirection());
            String outputFileName = "/files/temp/baocaohanghoatheoquanhuyen_" + ngayXuatBaoCao + ".xls";
            String reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/export/baocaohanghoatheoquanhuyen.xls");
            ...
            throw new Exception("Failed to export to excel BaoCaoHangHoaTheoQuanHuyen. \n" + e.getMessage());
        }
    }
