 ...
 */
@Controller
public class BaoCaoChiTietGoiKhuyenMai_TheoMaGoiVaTenGoi_TheoQuanHuyenController extends ApplicationObjectSupport {
    ...
    @Autowired
    private BaoCaoChiTietGoiKhuyenMai_TheoMaGoiTenGoiManagementLocalBean baoCaoChiTietGoiKhuenMai_TheoMaGoiTenGoiService;
    ...
    @RequestMapping(value = {"/admin/thuebaophattrienmoi/baocaochitietgoikm_theomagoivatengoi_theoquanhuyen.html",
            "/chinhanh/thuebaophattrienmoi/baocaochitietgoikm_theomagoivatengoi_theoquanhuyen.html",
            "/baocao/thuebaophattrienmoi/baocaochitietgoikm_theomagoivatengoi_theoquanhuyen.html"})
    public ModelAndView report(@ModelAttribute(value = Constants.FORM_MODEL_KEY)BaoCaoChiTietGoiKhuyenMaiCommand command,
        ...
        ModelAndView mav = new ModelAndView("2015/thuebaophattrienmoi/admin/baocaochitietgoikhuyenmai/theotengoi_magoi/theoquanhuyen");
        ...
        if (StringUtils.isNotBlank(crudaction)){
            if (crudaction.equals("search")){
                ...
                Object[] results = this.baoCaoChiTietGoiKhuenMai_TheoMaGoiTenGoiService.baoCaoChiTietGoiKhuyenMai_TheoMaGoi_TenGoi_TheoQuanHuyen(properties, command.getSortExpression(), command.getSortDirection(), command.getFirstItem(), command.getReportMaxPageItems());
    ...
    private void export2Excel(BaoCaoChiTietGoiKhuyenMaiCommand command, HttpServletRequest request, HttpServletResponse response) throws Exception {
        try{
            ...
            Map<String, Object> properties = buildProperties(command);
            Object[] resultObject = this.baoCaoChiTietGoiKhuenMai_TheoMaGoiTenGoiService.baoCaoChiTietGoiKhuyenMai_TheoMaGoi_TenGoi_TheoQuanHuyen(properties, command.getSortExpression(), command.getSortDirection(), 0, Integer.MAX_VALUE);
            List<BaoCaoChiTietGoiKhuyenMaiDTO> dtoList = (List<BaoCaoChiTietGoiKhuyenMaiDTO>)resultObject[1];

            String outputFileName = "/files/temp/baocaochitietgoikhuyenmai_theomagoivatengoi_theoquanhuyen" + ngayXuatBaoCao + ".xls";
            String reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/export/thuebaoptm_2015/baocaochitietgoikhuyenmai_theomagoivatengoi_theoquanhuyen.xls");
            ...
            if(command.getFromDate() != null){
                ...
                resValueNgayXuatBaoCaoTu[6] = new CellValue(CellDataType.STRING, "Ngày xuất báo cáo từ: " + df.format(command.getFromDate()));
                ExcelUtil.addRow(sheet, 4, resValueNgayXuatBaoCaoTu, stringNgayBaoCaoCellFormat, null, null, null);
            }
            if(command.getToDate() != null){
                CellValue[] resValueNgayXuatBaoCaoTu = WebCommonUtil.addNgayXuatBaoCao(TOTAL_COLUMN_EXPORT);
                resValueNgayXuatBaoCaoTu[6] = new CellValue(CellDataType.STRING, "Ngày xuất báo cáo đến: " + df.format(command.getToDate()));
    ...
    private CellValue[] addCellValues(BaoCaoChiTietGoiKhuyenMaiDTO dto, int indexRow) {
        ...
        SimpleDateFormat monthYearf = new SimpleDateFormat("M/yyyy");
        int columnIndex = 0;
        ...
        if(dto.getThoiGianDK() != null){
            resValue[columnIndex++] = new CellValue(CellDataType.STRING, monthYearf.format(dto.getThoiGianDK()));
        }else{
            resValue[columnIndex++] = new CellValue();
        }
