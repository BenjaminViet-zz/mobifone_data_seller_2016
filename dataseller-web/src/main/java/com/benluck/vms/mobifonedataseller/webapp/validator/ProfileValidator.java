 ...
 */
@Component
public class ProfileValidator extends ApplicationObjectSupport implements Validator {
    private Logger logger = Logger.getLogger(ProfileValidator.class);
    ...
    public void validate(Object o, Errors errors) {
        ...
        validProfileUserId(command, errors);
    ...
    private void checkRequiredFields(UserCommand command, Errors errors) {
        ...
        ValidationUtils.rejectIfEmptyOrWhitespace(errors, "pojo.displayName", "errors.required", new Object[]{this.getMessageSourceAccessor().getMessage("user.label.displayName")}, "non-empty value required.");
    }
    ...
    /**
     * Prevent hack update other UserId
     * @param command
     * @param errors
     */
    private void validProfileUserId(UserCommand command, Errors errors){
        if(!SecurityUtils.getLoginUserId().equals(command.getPojo().getUserId())){
            command.setErrorMessage(this.getMessageSourceAccessor().getMessage("hack_update_other_user_profile_warning"));
        }
    }
}
