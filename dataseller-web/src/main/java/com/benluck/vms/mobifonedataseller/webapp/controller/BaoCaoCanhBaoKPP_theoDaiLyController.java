...
import com.benluck.vms.mobifonedataseller.webapp.command.command2014.BaoCaoCanhBao_KPP_TheoDBHCommand;
 ...
 */
@Controller
public class BaoCaoCanhBaoKPP_theoDaiLyController extends ApplicationObjectSupport {
    private Log log = LogFactory.getLog(BaoCaoCanhBaoKPP_theoDaiLyController.class);
    private static final Integer TOTAL_COLUMN_EXPORT = 12;

    @Autowired
    private BaoCaoCanhBao_kpp_theoDBHManagementLocalBean reportManagementLocalBean;
    @Autowired
    ...
    @RequestMapping(value = {"/admin/baocaocanhbao_kpp_theodaily.html","/chinhanh/baocaocanhbao_kpp_theodaily.html"})
    public ModelAndView report(@ModelAttribute(value = Constants.FORM_MODEL_KEY)BaoCaoCanhBao_KPP_TheoDBHCommand command,
                               HttpServletRequest request,
                               HttpServletResponse response){
        ModelAndView mav = new ModelAndView("/admin/baocaocanhbao/theodaily");
        ...
        if(StringUtils.isNotBlank(crudaction)){
            if(crudaction.equals("search")){
                ...
                command.setListResult((List<BaoCaoCanhBao_KPP_TheoDBHDTO>)resultObject[1]);
     ...
     * @param mav
     */
    private void referenceData(BaoCaoCanhBao_KPP_TheoDBHCommand command, ModelAndView mav){
    ...
    /**
     * Get the Date value and format them to Timestamp for querying.
     * @param command
     */
    private void convertDate2Timestamp(BaoCaoCanhBao_KPP_TheoDBHCommand command){
     ...
     * @return
     */
    private Map<String, Object> buildProperties(BaoCaoCanhBao_KPP_TheoDBHCommand command){
     ...
     * @throws Exception
     */
    private void export2Excel(BaoCaoCanhBao_KPP_TheoDBHCommand command, HttpServletRequest request,HttpServletResponse response)throws Exception{
        try{
            ...
            List<BaoCaoCanhBao_KPP_TheoDBHDTO> dtoList = (List<BaoCaoCanhBao_KPP_TheoDBHDTO>)resultObject[1];
            String reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/export/baocaocanhbao_kpp_theodaily.xls");
            String outputFileName = "/files/temp/baocaocanhbao_kpp_theodaily_" + ngayXuatBaoCao + ".xls";
            ...
            WritableCellFormat doubleCellFormat = new WritableCellFormat(normalFont);
            ...
            if(dtoList.size() > 0){
                int indexRow = 1;
                for(BaoCaoCanhBao_KPP_TheoDBHDTO dto : dtoList){
            ...
            throw new Exception("Failed to export to excel BaoCaoCanhBaoKPPTheoDaiLy. \n" + e.getMessage());
        }
    }
     ...
     * @return
     */
    private CellValue[] addCellValues(BaoCaoCanhBao_KPP_TheoDBHDTO dto, int indexRow){
        SimpleDateFormat df = new SimpleDateFormat("dd-M-yyyy");
        ...
        if(dto.getNgayTongHop() != null){
            resValue[columnIndex++] = new CellValue(CellDataType.STRING, df.format(dto.getNgayTongHop()));
        }else{
            resValue[columnIndex++] = new CellValue();
        }
