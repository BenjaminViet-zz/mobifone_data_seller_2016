 ...
 */
@Controller
public class BaoCaoChiTietGoiKhuyenMai_TheoKhachHang_TheoChiNhanhController extends ApplicationObjectSupport{
    private Log log = LogFactory.getLog(BaoCaoChiTietGoiKhuyenMai_TheoKhachHang_TheoChiNhanhController.class);
    ...
    @RequestMapping(value = {"/admin/thuebaophattrienmoi/baocaochitietgoikm_theoKhachHang_theochinhanh.html",
            "/chinhanh/thuebaophattrienmoi/baocaochitietgoikm_theoKhachHang_theochinhanh.html",
            "/baocao/thuebaophattrienmoi/baocaochitietgoikm_theoKhachHang_theochinhanh.html"})
    public ModelAndView report(@ModelAttribute(value = Constants.FORM_MODEL_KEY)BaoCaoChiTietGoiKhuyenMaiCommand command,
        ...
        ModelAndView mav = new ModelAndView("2015/thuebaophattrienmoi/admin/baocaochitietgoikhuyenmai/theosothuebaokhachhang/theochinhanh");
        ...
        if (StringUtils.isNotBlank(crudaction)){
            if (crudaction.equals("search")){
                ...
                Object[] results = this.baoCaoChiTietGoiKhuyenMai_TheoKhachHangService.baoCaoChiTietGoiKhuyenMai_TheoKhachHang_TheoChiNhanh(properties, command.getSortExpression(), command.getSortDirection(), command.getFirstItem(), command.getMaxPageItems());
    ...
    private void export2Excel(BaoCaoChiTietGoiKhuyenMaiCommand command, HttpServletRequest request, HttpServletResponse response) throws Exception {
        try{
            ...
            Map<String, Object> properties = buildProperties(command);
            Object[] resultObject = this.baoCaoChiTietGoiKhuyenMai_TheoKhachHangService.baoCaoChiTietGoiKhuyenMai_TheoKhachHang_TheoQuanHuyen(properties, command.getSortExpression(), command.getSortDirection(), 0, Integer.MAX_VALUE);
            List<BaoCaoChiTietGoiKhuyenMaiDTO> dtoList = (List<BaoCaoChiTietGoiKhuyenMaiDTO>)resultObject[1];

            String outputFileName = "/files/temp/baocaochitietgoikhuyenmai_theokhachhang_theochinhanh" + ngayXuatBaoCao + ".xls";
            String reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/export/thuebaoptm_2015/baocaochitietgoikhuyenmai_theokhachhang_theochinhanh.xls");
