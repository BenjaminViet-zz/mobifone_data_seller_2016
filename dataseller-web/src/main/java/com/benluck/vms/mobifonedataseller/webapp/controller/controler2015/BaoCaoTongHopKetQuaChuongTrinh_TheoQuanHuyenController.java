 ...
 */
@Controller
public class BaoCaoTongHopKetQuaChuongTrinh_TheoQuanHuyenController extends ApplicationObjectSupport{
    private Log log = LogFactory.getLog(BaoCaoTongHopKetQuaChuongTrinh_TheoQuanHuyenController.class);
    private static final Integer TOTAL_COLUMN_EXPORT = 18;
    ...
    @RequestMapping(value = {"/admin/thuebaophattrienmoi/baocaotonghop_ketquachuongtrinh_theoquanhuyen.html",
            "/chinhanh/thuebaophattrienmoi/baocaotonghop_ketquachuongtrinh_theoquanhuyen.html",
            "/baocao/thuebaophattrienmoi/baocaotonghop_ketquachuongtrinh_theoquanhuyen.html"})
    public ModelAndView report(@ModelAttribute(value = Constants.FORM_MODEL_KEY)BaoCaoTongHopKetQuaChuongTrinhCommand command,
        ...
        ModelAndView mav = new ModelAndView("2015/thuebaophattrienmoi/admin/baocaotonghopketquachuongtrinh/theoquanhuyen");
        ...
        if (StringUtils.isNotBlank(crudaction)){
            if (crudaction.equals("search")){
                ...
                Object[] results = this.baoCaoTongHopKetQuaChuongTrinhService.baoCaoTongHopKetQuaChuongTrinh_TheoQuanHuyen(properties, command.getSortExpression(), command.getSortDirection(), command.getFirstItem(), command.getReportMaxPageItems());
    ...
    private void export2Excel(BaoCaoTongHopKetQuaChuongTrinhCommand command, HttpServletRequest request, HttpServletResponse response) throws Exception {
        try{
            ...
            Map<String, Object> properties = buildProperties(command);
            Object[] resultObject = this.baoCaoTongHopKetQuaChuongTrinhService.baoCaoTongHopKetQuaChuongTrinh_TheoQuanHuyen(properties, command.getSortExpression(), command.getSortDirection(), 0, Integer.MAX_VALUE);
            List<BaoCaoTongHopKetQuaChuongTrinhDTO> dtoList = (List<BaoCaoTongHopKetQuaChuongTrinhDTO>)resultObject[1];

            String outputFileName = "/files/temp/baocaotonghopketquachuongtrinh_theoquanhuyen" + ngayXuatBaoCao + ".xls";
            String reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/export/thuebaoptm_2015/baocaotonghopketquachuongtrinh_theoquanhuyen.xls");
    ...
    private CellValue[] addCellValues(BaoCaoTongHopKetQuaChuongTrinhDTO dto, int indexRow) {
        ...
        if(dto.getSLDBHThamGia() != null){
            resValue[columnIndex++] = new CellValue(CellDataType.INT, dto.getSLDBHThamGia());
        }else{
            resValue[columnIndex++] = new CellValue();
        }
