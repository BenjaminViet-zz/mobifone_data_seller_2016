 ...
 */
@Controller
public class BaoCaoChiTietGoiKhuyenMai_TheoMaGoiVaTenGoi_TheoDailyController extends ApplicationObjectSupport {
    private Log log = LogFactory.getLog(BaoCaoChiTietGoiKhuyenMai_TheoMaGoiVaTenGoi_TheoDailyController.class);
    private static final Integer TOTAL_COLUMN_EXPORT = 14;
    ...
    @RequestMapping(value = {"/admin/thuebaophattrienmoi/baocaochitietgoikm_theomagoivatengoi_theodaily.html",
            "/chinhanh/thuebaophattrienmoi/baocaochitietgoikm_theomagoivatengoi_theodaily.html",
            "/baocao/thuebaophattrienmoi/baocaochitietgoikm_theomagoivatengoi_theodaily.html"})
    public ModelAndView report(@ModelAttribute(value = Constants.FORM_MODEL_KEY)BaoCaoChiTietGoiKhuyenMaiCommand command,
        ...
        ModelAndView mav = new ModelAndView("2015/thuebaophattrienmoi/admin/baocaochitietgoikhuyenmai/theotengoi_magoi/theodaily");
        ...
        if (StringUtils.isNotBlank(crudaction)){
            if (crudaction.equals("search")){
                ...
                Object[] results = this.baoCaoChiTietGoiKhuyenMai_TheoMaGoiTenGoiService.baoCaoChiTietGoiKhuyenMai_TheoMaGoi_TenGoi_TheoDiemBanHang(properties, command.getSortExpression(), command.getSortDirection(), command.getFirstItem(), command.getReportMaxPageItems());
    ...
    private void export2Excel(BaoCaoChiTietGoiKhuyenMaiCommand command, HttpServletRequest request, HttpServletResponse response) throws Exception {
        try{
            ...
            Map<String, Object> properties = buildProperties(command);
            Object[] resultObject = this.baoCaoChiTietGoiKhuyenMai_TheoMaGoiTenGoiService.baoCaoChiTietGoiKhuyenMai_TheoMaGoi_TenGoi_TheoDiemBanHang(properties, command.getSortExpression(), command.getSortDirection(), 0, Integer.MAX_VALUE);
            List<BaoCaoChiTietGoiKhuyenMaiDTO> dtoList = (List<BaoCaoChiTietGoiKhuyenMaiDTO>)resultObject[1];

            String outputFileName = "/files/temp/baocaochitietgoikhuyenmai_theomagoivatengoi_theodaily" + ngayXuatBaoCao + ".xls";
            String export2FileName = request.getSession().getServletContext().getRealPath(outputFileName);
            String reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/export/thuebaoptm_2015/baocaochitietgoikhuyenmai_theomagoivatengoi_theodaily.xls");
            ...
            if(command.getFromDate() != null){
                ...
                resValueNgayXuatBaoCaoTu[11] = new CellValue(CellDataType.STRING, "Ngày xuất báo cáo từ: " + df.format(command.getFromDate()));
                ExcelUtil.addRow(sheet, 4, resValueNgayXuatBaoCaoTu, stringNgayBaoCaoCellFormat, null, null, null);
            }
            if(command.getToDate() != null){
                CellValue[] resValueNgayXuatBaoCaoTu = WebCommonUtil.addNgayXuatBaoCao(TOTAL_COLUMN_EXPORT);
                resValueNgayXuatBaoCaoTu[11] = new CellValue(CellDataType.STRING, "Ngày xuất báo cáo đến: " + df.format(command.getToDate()));
