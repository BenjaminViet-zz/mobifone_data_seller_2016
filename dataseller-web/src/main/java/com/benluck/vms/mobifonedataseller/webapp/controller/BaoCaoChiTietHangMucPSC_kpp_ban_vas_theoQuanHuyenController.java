...
import com.benluck.vms.mobifonedataseller.webapp.command.command2014.BaoCaoChiTietHangMucPSC_kpp_ban_vas_theoQuanHuyenCommand;
 ...
 */
@Controller
public class BaoCaoChiTietHangMucPSC_kpp_ban_vas_theoQuanHuyenController extends ApplicationObjectSupport {
    private Log log = LogFactory.getLog(BaoCaoChiTietHangMucPSC_kpp_ban_vas_theoQuanHuyenController.class);
    private static final Integer TOTAL_COLUMN_EXPORT = 6;

    @Autowired
    private BaoCaoChiTietHangMucPSC_kpp_ban_vas_theoQuanHuyen_managementLocalBean reportManagementLocalBean;
    @Autowired
    ...
    @RequestMapping(value = {"/admin/baocaochitiethangmucpsc_ban_vas_theoquanhuyen.html","/chinhanh/baocaochitiethangmucpsc_ban_vas_theoquanhuyen.html"})
    public ModelAndView report(@ModelAttribute(value = Constants.FORM_MODEL_KEY)BaoCaoChiTietHangMucPSC_kpp_ban_vas_theoQuanHuyenCommand command,
                               HttpServletRequest request,
                               HttpServletResponse response){
        ModelAndView mav = new ModelAndView("/admin/baocaochitiethangmucpsc/banvas/theoquanhuyen");
        ...
        if(StringUtils.isNotBlank(crudaction)){
            if(crudaction.equals("search")){
                ...
                command.setListResult((List<BaoCaoChiTietHangMucPSCKPP_ban_vas_TheoQuanHuyenDTO>)resultObject[1]);
    ...
    private void referenceData(BaoCaoChiTietHangMucPSC_kpp_ban_vas_theoQuanHuyenCommand command, ModelAndView mav){
    ...
    private Map<String, Object> buildProperties(BaoCaoChiTietHangMucPSC_kpp_ban_vas_theoQuanHuyenCommand command){
    ...
    private void convertDate2Timestamp(BaoCaoChiTietHangMucPSC_kpp_ban_vas_theoQuanHuyenCommand command){
    ...
    private void export2Excel(BaoCaoChiTietHangMucPSC_kpp_ban_vas_theoQuanHuyenCommand command, HttpServletRequest request,HttpServletResponse response)throws Exception{
        try{
            ...
            List<BaoCaoChiTietHangMucPSCKPP_ban_vas_TheoQuanHuyenDTO> dtoList = (List<BaoCaoChiTietHangMucPSCKPP_ban_vas_TheoQuanHuyenDTO>)resultObject[1];
            String reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/export/baocaochitiethangmuc_psc_kpp_ban_vas_theoquanhuyen.xls");
            String outputFileName = "/files/temp/baocaochitiethangmuc_psc_kpp_ban_vas_theoquanhuyen_" + ngayXuatBaoCao + ".xls";
            ...
            if(dtoList.size() > 0){
                int indexRow = 1;
                for(BaoCaoChiTietHangMucPSCKPP_ban_vas_TheoQuanHuyenDTO dto : dtoList){
    ...
    private CellValue[] addCellValues(BaoCaoChiTietHangMucPSCKPP_ban_vas_TheoQuanHuyenDTO dto, int indexRow){
        SimpleDateFormat df = new SimpleDateFormat("dd-M-yyyy");
