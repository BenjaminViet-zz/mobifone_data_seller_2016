 ...
 */
@Controller
public class BaoCaoHangHoaTheoDaiLyController extends ApplicationObjectSupport{
    private Log log = LogFactory.getLog(BaoCaoHangHoaTheoDaiLyController.class);
    ...
    @RequestMapping(value = {"/admin/baocaohanghoatheodaily.html","/chinhanh/baocaohanghoatheodaily.html"})
    public ModelAndView baocaohanghoatheodaily(@ModelAttribute(value = Constants.FORM_MODEL_KEY)BaoCaoHangHoaCommand command,
        ...
        ModelAndView mav = new ModelAndView("/admin/baocaohanghoatheodaily");
        ...
        if(StringUtils.isNotBlank(crudaction)){
            if(crudaction.equals("search")){
                ...
                Object[] resultObject = this.baoCaoHangHoaManagementLocalBean.baoCaoHangHoaTheoDaiLy(properties, command.getFirstItem(), command.getReportMaxPageItems(), command.getSortExpression(), command.getSortDirection());
                command.setTotalItems(Integer.valueOf(resultObject[0].toString()));
                command.setListResult((List<BaoCaoHangHoaDTO>) resultObject[1]);
        ...
        if(SecurityUtils.userHasAuthority(Constants.ADMIN_ROLE) || SecurityUtils.userHasAuthority(Constants.BAOCAO_ROLE)){
            ...
            List<DistrictDTO> districtList = this.baoCaoHangHoaManagementLocalBean.findByBranchId(SecurityUtils.getPrincipal().getBranchId());
            mav.addObject("districtList", districtList);
        }


        if(command.getBranch_Id() != null){
            List<DistrictDTO> districtList = this.baoCaoHangHoaManagementLocalBean.findByBranchId(command.getBranch_Id());
            mav.addObject("districtList", districtList);
        }
        if(command.getDistrict_Id() != null){
            List<RetailDealerDTO> dealerDTOs = this.baoCaoHangHoaManagementLocalBean.findRetailDealerList(null, command.getDistrict_Id());
            mav.addObject("retailDealerList", dealerDTOs);
        }
    ...
    private void export2Excel(BaoCaoHangHoaCommand command, HttpServletRequest request,HttpServletResponse response)throws Exception{
        try{
            ...
            convertDate2Timestamp(command);
            Map<String, Object> properties = buildProperties(command);
            Object[] resultObject = this.baoCaoHangHoaManagementLocalBean.baoCaoHangHoaTheoDaiLy(properties, 0, Integer.MAX_VALUE, command.getSortExpression(), command.getSortDirection());
            String outputFileName = "/files/temp/baocaohanghoatheodaily_" + ngayXuatBaoCao + ".xls";
            String reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/export/baocaohanghoatheodaily.xls");
            ...
            List<BaoCaoHangHoaDTO> dtoList = (List<BaoCaoHangHoaDTO>)resultObject[1];
            if(dtoList.size() > 0){
                int indexRow = 1;
            ...
            throw new Exception("Failed to export to excel BaoCaoHangHoaTheoDaiLy. \n" + e.getMessage());
        }
    }
