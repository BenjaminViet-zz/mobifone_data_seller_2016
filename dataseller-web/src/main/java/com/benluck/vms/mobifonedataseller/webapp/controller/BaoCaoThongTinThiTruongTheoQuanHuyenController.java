...
@Controller
public class BaoCaoThongTinThiTruongTheoQuanHuyenController extends ApplicationObjectSupport {
    ...
    private Log log = LogFactory.getLog(BaoCaoThongTinThiTruongTheoQuanHuyenController.class);
    ...
    @RequestMapping(value = {"/admin/baocaothongtinthitruongtheoquanhuyen.html","/chinhanh/baocaothongtinthitruongtheoquanhuyen.html"})
    public ModelAndView baocaothongtinthitruong(@ModelAttribute(value = Constants.FORM_MODEL_KEY)BaoCaoThongTinThiTruongCommand command,
        ...
        ModelAndView mav = new ModelAndView("/admin/baocaothongtinthitruongtheoquanhuyen");
        ...
        if(StringUtils.isNotBlank(crudaction)){
            if(crudaction.equals("search")){
                ...
                Object[] resultObject = this.baoCaoThongTinThiTruongManagementLocalBean.baoCaoThongTinThiTruongTheoQuanHuyen(properties, command.getFirstItem(), command.getReportMaxPageItems(), command.getSortExpression(), command.getSortDirection());
    ...
    private void export2Excel(BaoCaoThongTinThiTruongCommand command, HttpServletRequest request, HttpServletResponse response) throws Exception{
        try{
            ...
            convertDate2Timestamp(command);
            Map<String, Object> properties = buildProperties(command);
            Object[] resultObject = this.baoCaoThongTinThiTruongManagementLocalBean.baoCaoThongTinThiTruongTheoQuanHuyen(properties, 0, Integer.MAX_VALUE, command.getSortExpression(), command.getSortDirection());
            List<BaoCaoTTThiTruongDTO> dtoList = (List<BaoCaoTTThiTruongDTO>)resultObject[1];
            String outputFileName = "/files/temp/baocaothongtinthitruongtheoquanhuyen_" + ngayXuatBaoCao + ".xls";
            String reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/export/baocaothongtinthitruongtheoquanhuyen.xls");
