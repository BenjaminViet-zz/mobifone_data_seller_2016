 ...
 */
@Controller
public class BaoCaoChiTraKhuyenKhichChoDiemBanHang_TheoQuanHuyenController extends ApplicationObjectSupport {
    ...
    @RequestMapping(value = {"/admin/thuebaophattrienmoi/baocaochitrakhuyenkhich_cho_dbh_theoquanhuyen.html",
            "/chinhanh/thuebaophattrienmoi/baocaochitrakhuyenkhich_cho_dbh_theoquanhuyen.html",
            "/baocao/thuebaophattrienmoi/baocaochitrakhuyenkhich_cho_dbh_theoquanhuyen.html"})
    public ModelAndView report(@ModelAttribute(value = Constants.FORM_MODEL_KEY)BaoCaoChiTraKhuyenKhichChoDBHCommand command,
        ...
        ModelAndView mav = new ModelAndView("2015/thuebaophattrienmoi/admin/baocaochitrakhuyenkhichchodbh/theoquanhuyen");
        ...
        if (StringUtils.isNotBlank(crudaction)){
            if (crudaction.equals("search")){
                ...
                Object[] results = this.BaoCaoChiTraKhuyenKhichChoDiemBanHangService.baoCaoChiTraKhuyenKhichChoDiemBanHang_TheoQuanHuyen(properties, command.getSortExpression(), command.getSortDirection(), command.getFirstItem(), command.getMaxPageItems());
    ...
    private void export2Excel(BaoCaoChiTraKhuyenKhichChoDBHCommand command, HttpServletRequest request, HttpServletResponse response) throws Exception {
        try{
            ...
            Map<String, Object> properties = buildProperties(command);
            Object[] resultObject = this.BaoCaoChiTraKhuyenKhichChoDiemBanHangService.baoCaoChiTraKhuyenKhichChoDiemBanHang_TheoQuanHuyen(properties, command.getSortExpression(), command.getSortDirection(), 0, Integer.MAX_VALUE);
            List<BaoCaoChiTraKhuyenKhichChoDBHDTO> dtoList = (List<BaoCaoChiTraKhuyenKhichChoDBHDTO>)resultObject[1];

            String outputFileName = "/files/temp/baocaochitrakhuyenkhich_theoquanhuyen" + ngayXuatBaoCao + ".xls";
            String reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/export/thuebaoptm_2015/baocaochitrakhuyenkhich_theoquanhuyen.xls");
