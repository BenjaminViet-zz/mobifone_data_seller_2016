 ...
 */
@Controller
public class BaoCaoTienThuongMaDuThuongTheoQuanHuyenController extends ApplicationObjectSupport{
    ...
    @RequestMapping(value = {"/admin/baocaotienthuongmaduthuongtheoquanhuyen.html","/chinhanh/baocaotienthuongmaduthuongtheoquanhuyen.html"})
    public ModelAndView baoCaoTienThuongMaDuThuong(@ModelAttribute(value = Constants.FORM_MODEL_KEY)BaoCaoTienThuongMaDuThuongCommand command,
        ...
        ModelAndView mav = new ModelAndView("/admin/baocaotienthuongmaduthuongtheoquanhuyen");
        ...
        if(StringUtils.isNotBlank(crudaction)){
            if(crudaction.equals("search")){
                ...
                Object[] resultObject = this.baoCaoTienThuongMaDuThuongManagementLocalBean.baoCaoTienThuongMaDuThuongTheoQuanHuyen(properties, command.getFirstItem(), command.getReportMaxPageItems(), command.getSortExpression(), command.getSortDirection());
        ...
        if(command.getBranch_Id() != null){
            List<DistrictDTO> districtList = this.baoCaoTienThuongMaDuThuongManagementLocalBean.findByBranchId(command.getBranch_Id());
            mav.addObject("districtList", districtList);
        }
        if(command.getDistrict_Id() != null){
            List<RetailDealerDTO> dealerDTOs = this.baoCaoTienThuongMaDuThuongManagementLocalBean.findRetailDealerList(null, command.getDistrict_Id());
    ...
    private void export2Excel(BaoCaoTienThuongMaDuThuongCommand command, HttpServletRequest request,HttpServletResponse response)throws Exception{
        try{
            ...
            convertDate2Timestamp(command);
            Map<String, Object> properties = buildProperties(command);
            Object[] resultObject = this.baoCaoTienThuongMaDuThuongManagementLocalBean.baoCaoTienThuongMaDuThuongTheoQuanHuyen(properties, 0, Integer.MAX_VALUE, command.getSortExpression(), command.getSortDirection());
            List<BaoCaoTienThuongMaDuThuongDTO> dtoList = (List<BaoCaoTienThuongMaDuThuongDTO>)resultObject[1];
            String outputFileName = "/files/temp/baocaotienthuongmaduthuongtheoquanhuyen_" + ngayXuatBaoCao + ".xls";
            String reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/export/baocaotienthuongmaduthuongtheoquanhuyen.xls");
            ...
            throw new Exception("Failed to export to excel BaoCaoTienThuongMaDuThuongTheoQuanHuyen. \n" + e.getMessage());
        }
    }
