 ...
 */
@Controller
public class BaoCaoTinNhanTheoChiNhanhController extends ApplicationObjectSupport {
    private Log log = LogFactory.getLog(BaoCaoTinNhanTheoChiNhanhController.class);
    ...
    @RequestMapping(value = {"/admin/thuebaophattrienmoi/baocaotinnhan_theochinhanh.html",
            "/chinhanh/thuebaophattrienmoi/baocaotinnhan_theochinhanh.html",
            "/baocao/thuebaophattrienmoi/baocaotinnhan_theochinhanh.html"})
    public ModelAndView report(@ModelAttribute(value = Constants.FORM_MODEL_KEY)BaoCaoTinNhanCommand command,
        ...
        ModelAndView mav = new ModelAndView("2015/thuebaophattrienmoi/admin/baocaotinnhan/theochinhanh");
        ...
        if (StringUtils.isNotBlank(crudaction)){
            if (crudaction.equals("search")){
                ...
                Object[] results = this.baoCaoTinNhanService.baoCaoTinNhan_TheoChiNhanh(properties, command.getSortExpression(), command.getSortDirection(), command.getFirstItem(), command.getReportMaxPageItems());
    ...
    private void export2Excel(BaoCaoTinNhanCommand command, HttpServletRequest request, HttpServletResponse response) throws Exception {
        try{
            ...
            Map<String, Object> properties = buildProperties(command);
            Object[] resultObject = this.baoCaoTinNhanService.baoCaoTinNhan_TheoChiNhanh(properties, command.getSortExpression(), command.getSortDirection(), 0, Integer.MAX_VALUE);
            List<BaoCaoTinNhanDTO> dtoList = (List<BaoCaoTinNhanDTO>)resultObject[1];

            String outputFileName = "/files/temp/baocaotinnhan_theochinhanh" + ngayXuatBaoCao + ".xls";
            String reportTemplate = request.getSession().getServletContext().getRealPath("/files/temp/export/thuebaoptm_2015/baocaotinnhan_theochinhanh.xls");
